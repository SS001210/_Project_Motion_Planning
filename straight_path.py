...
def generate_s_trajectory(x0, y0, parkx, parky, parking_spot_length, parking_spot_breath, offset=15, m0=0 , mf=0 , k0=0, kf=0, num_points=15):
    """
...
def main():
    ...
    # Initial state for forward parking
      # Parking speed [m/s]
    f_accel = -3.8  # Parking acceleration [m/s^2]
    ...
    for i in range(SIM_LOOP):
        if state == 'motion':
            ...
            f_speed = path.s_d[1]
            f_accel = path.s_dd[1]
            if np.hypot(path.x[1] - tx[-1], path.y[1] - ty[-1]) <= 0.1:
                break
        ...
        if show_animation:
            ...
            print(f"px: {px:.2f}, py: {py:.2f}, yaw: {yaw:.2f}")  # Debugging infoprint(f"px: {px}, py: {py}, yaw: {yaw}")  # Debugging info
