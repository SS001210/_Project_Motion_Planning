...
# Parameter
MAX_SPEED = 50.0 / 3.6  # mpltimum speed [m/s]
...
TARGET_SPEED = 0.0 / 3.6  # target speed [m/s]
...
def main():
    ...
    f_accel = 0.0  # Parking acceleration [m/s^2]
    ...
    for i in range(SIM_LOOP):
        ...
        if state == 'motion':
            path,fplist = frenet_optimal_planning(csp, s0, c_speed, c_accel, c_d, c_d_d, c_d_dd)
            s0 = path.s[1]
            c_d = path.d[1]
            c_d_d = path.d_d[1]
            c_d_dd = path.d_dd[1]
            ...
            path,fplist = frenet_optimal_planning(csp, fs0, f_speed, f_accel, f_d, f_d_d, f_d_dd)
            fs0 = path.s[1]
            f_d = path.d[1]
            f_d_d = path.d_d[1]
            f_d_dd = path.d_dd[1]
