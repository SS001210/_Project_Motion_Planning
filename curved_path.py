...
initial_speed = 50.0 / 3.6  # target speed of moition [m/s]
...
parky = -20.0 # y coordinate for the parking 
pspot_length = 6.0
pspot_breadth = 4.0
# cost weights
K_J = 0.1
...
def plot_road_boundaries(wx, wy, road_width):

    """ Plot road boundaries with respect to wx, wy reference coordinates and smooth curvature.
    Parameters:
        wx, wy (list): The reference coordinates for the road.
        road_width (float): The width of the road."""
    
    # Generate the cubic spline for the given wx, wy coordinates
    csp = cubic_spline_planner.CubicSpline2D(wx, wy)
    s = np.arange(0, csp.s[-1], 0.1)

    road_left_x, road_left_y, road_right_x, road_right_y = [], [], [], []

    # Calculate the road boundaries (left and right edges)
    for i_s in s:
        ix, iy = csp.calc_position(i_s)
        iyaw = csp.calc_yaw(i_s)
        left_x = ix + road_width / 2.0 * math.cos(iyaw + math.pi / 2.0)
        left_y = iy + road_width / 2.0 * math.sin(iyaw + math.pi / 2.0)
        right_x = ix - road_width / 2.0 * math.cos(iyaw + math.pi / 2.0)
        right_y = iy - road_width / 2.0 * math.sin(iyaw + math.pi / 2.0)

        road_left_x.append(left_x)
        road_left_y.append(left_y)
        road_right_x.append(right_x)
        road_right_y.append(right_y)

    # Plot the road boundaries
    plt.plot(road_left_x, road_left_y, 'k', linewidth=2)
    plt.plot(road_right_x, road_right_y, 'k', linewidth=2)

def generate_s_trajectory(x0, y0, parkx, parky, parking_spot_length, parking_spot_breath, offset=15, m0= -0.5, mf= 0.05 , k0=0, kf=0, num_points=25):
    """
...
def main():
    ...
    wy = [0.0, 7.07, 10.0, 7.07, 0.0, -7.07, -10.0, -7.07, 0.0]
    tx, ty, tyaw, tc, csp = generate_target_course(wx, wy) 
    # Initial state for forward motion
    c_speed = initial_speed  # Current speed [m/s]
    ...
    # Initial state for forward parking
      # Parking speed [m/s]
    f_accel = -0.05  # Parking acceleration [m/s^2]
    ...
    for i in range(SIM_LOOP):
        if state == 'motion':
            ...
            if state == "motion" and (s0 >= parkx - 15.0) :
                state = "forward"
                ...
                if is_parallel:
                    ...
                    if parky < 0 :  
                        fvx = [ego_x, ego_x , parkx-2, parkx-1, parkx]
        ...
        if show_animation:
            ...
            plt.plot(tx, ty)
            plot_road_boundaries(wx, wy, road_width)
            ...
            print(f"px: {px:.2f}, py: {py:.2f}, yaw: {yaw:.1f}")  # Debugging infoprint(f"px: {px}, py: {py}, yaw: {yaw}")  # Debugging info

            #Plot the Top 10 paths
            for i, fp in enumerate(fplist[:50]):  #inlky the top 10 paths
