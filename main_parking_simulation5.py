...
TARGET_SPEED = 30.0 / 3.6  # target speed of moition [m/s]
...
def calc_frenet_paths(c_speed, c_accel, c_d, c_d_d, c_d_dd, s0, state):
    ...
    for di in np.arange(-MAX_ROAD_WIDTH, MAX_ROAD_WIDTH, D_ROAD_W):
        ...
        for Ti in np.arange(MIN_T, MAX_T, DT):
            ...
            current_target_speed = c_speed if state == "motion" else v1
...
def main():
    ...
    for i in range(SIM_LOOP):
        ...
        if show_animation:
            ...
            plt.title(f"Straight speed[km/h]: {c_speed * 3.6:.2f}, park speed[km/h]: {f_speed*3.6:.2f}")
            plt.grid(True)
            plt.pause(0.04)
            #print("GOAL2")
